...
export default function DriverPortfolioPage() {
  ...
  //         // Ako su vozila popunjena kao objekti, uzmemo samo _id
  //         // Ako su već ID-ovi, koristimo ih direktno
  //         const vehicleIds =
  //           portfolioData.vehicles && portfolioData.vehicles.length > 0
  //             ? portfolioData.vehicles.map((vehicle) =>
  //                 typeof vehicle === "object" ? vehicle._id : vehicle
  //               )
  //             : [];

  //         // FILTRIRAJ: Ukloni vozila koja više ne postoje
  //         const validVehicleIds = vehicleIds.filter((vehicleId) =>
  //           userVehicles.some((v) => v._id === vehicleId)
  //         );

  //         console.log("Učitana vozila iz portfolija:", vehicleIds);

  //         setPortfolio(portfolioData);
  //         setFormData({
  //           ...portfolioData,
  //           vehicles: vehicleIds,
  //           // Ostavite firstName i lastName kako su sačuvani u bazi
  //           contactInfo: response.data.portfolio.contactInfo || {
  //             phone: "",
  //             email: "",
  //           },
  //           slug:
  //             response.data.portfolio.slug ||
  //             generateSlug(
  //               `${response.data.portfolio.firstName || ""} ${
  //                 response.data.portfolio.lastName || ""
  //               }`.trim() ||
  //                 user?.name ||
  //                 ""
  //             ),
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          {/* Moja Vozila */}
          {/* Moja Vozila sekcija - ISPRAVLJENO */}
                ...
                <div className="space-y-3">
                  ...
                  <div className="mt-4 p-3 bg-gray-100 rounded-lg">
                    <p className="text-sm font-medium">
                      Trenutno odabrana vozila (ID-ovi):
                    </p>
                    <p className="text-xs text-gray-600">
                      {formData.vehicles.join(", ") || "Nema odabranih vozila"}
                    </p>
                  </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  portfolio.vehicles.length > 0 ? (
                    portfolio.vehicles.map((vehicle) => {
                      // Ako je vehicle string (ID), pronađi kompletne podatke
                      if (typeof vehicle === "string") {
                        const vehicleData = userVehicles.find(
                          (v) => v._id === vehicle
                        );
                        return vehicleData ? (
                          <div
                          ...
                          >
                            ...
                            <div className="text-sm text-gray-600 space-y-1">
                              <div>
                                Registracija: {vehicleData.licensePlate}
                              </div>
                              <div>Kapacitet: {vehicleData.capacity} kg</div>
                              {vehicleData.brand && (
                                <div>Marka: {vehicleData.brand}</div>
                              )}
                              {vehicleData.model && (
                                <div>Model: {vehicleData.model}</div>
                              )}
                              {vehicleData.year && (
                                <div>Godina: {vehicleData.year}</div>
                              )}
                            </div>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-sm text-gray-600 space-y-1">
                              ...
                              {vehicle.brand && (
                                <div>Marka: {vehicle.brand}</div>
                              )}
                              {vehicle.model && (
                                <div>Model: {vehicle.model}</div>
                              )}
                              {vehicle.year && (
                                <div>Godina: {vehicle.year}</div>
                              )}
                            </div>
                            {vehicle.description && (
                              <p className="text-gray-600 text-sm mt-3 pt-2 border-t border-gray-200">

...
export default function Pricing() {
  ...
  function formatPrice(plan) {
    ...
    return price === 0 ? "Free" : `${price} € ${billing === "monthly" ? "/mo" : "/yr"}`;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <header className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Izaberi ulogu i paket koji odgovara tvom poslovanju.</p>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-4 sm:items-center sm:justify-between">
          <div className="bg-white border rounded-lg shadow-sm p-1 flex items-center w-fit">
            <button
              ...
              className={`px-4 py-2 rounded-md font-medium transition ${role === "prevoznik" ? "bg-blue-600 text-white" : "text-gray-600 hover:text-gray-800"}`}>
              Prevoznici
            </button>
            <button
              onClick={() => setRole("potrazivac")}
              className={`px-4 py-2 rounded-md font-medium transition ${role === "potrazivac" ? "bg-blue-600 text-white" : "text-gray-600 hover:text-gray-800"}`}>
              Potraživaoci
            </button>
          </div>

          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-600">{billing === "monthly" ? "Mesečno" : "Godišnje"}</span>
            <button
              onClick={() => setBilling(billing === "monthly" ? "yearly" : "monthly")}
              className="px-3 py-1 bg-blue-600 text-white rounded-md text-sm shadow hover:bg-blue-700 transition">
              {billing === "monthly" ? "Prebaci na godišnje" : "Prebaci na mesečno"}
            </button>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {plans[role].map((plan) => (
          <div key={plan.id} className="bg-white border rounded-lg p-6 shadow hover:shadow-md transition">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-800">{plan.title}</h2>
              <div className="text-right">
                <div className="text-2xl font-bold text-blue-600">{formatPrice(plan)}</div>
                {plan.id === "pro" && billing === "yearly" && <div className="text-xs text-green-600">Uštedi {Math.round((plan.priceMonthly * 12 - plan.priceYearly))} €</div>}
              </div>
            </div>
            ...
            <div className="mt-6 flex items-center justify-between">
              <a className="px-4 py-2 bg-gray-100 border rounded-md text-sm hover:bg-gray-200 transition">Detalji</a>
              <button className={`px-5 py-2 rounded-md font-medium shadow ${plan.id === 'free' ? 'bg-gray-200 text-gray-700 hover:bg-gray-300' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>
